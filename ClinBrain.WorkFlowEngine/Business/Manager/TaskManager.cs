...
namespace ClinBrain.WorkFlowEngine.Business.Manager
{
    ...
    public class TaskManager : ManagerBase
    {
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        internal IEnumerable<TaskViewEntity> GetMyCompleteTasks(TaskQueryEntity query, out int allRowsCount)
        {
        ...
        private IEnumerable<TaskViewEntity> GetTasksPaged(TaskQueryEntity query, int activityState, out int allRowsCount)
        {
            ...
            //DATEDIFF(s,CreatedDateTime,EndedDateTime) >5 为了排队申请后立刻出现在我的完成里 ，判定是机器批的完成，机器批的完成都是第一步申请的时候
            ...
            sql.Append(@"SELECT 
                        ...
                        ");
            if (activityState == 4)
                sql.Append(" AND DATEDIFF(s,CreatedDateTime,EndedDateTime) >5");
            ...
            IDbSession session)
        {
            ...
            runner.CurrentStepName = task.ActivityName;   //kevin 0629
