    ...
    internal class NodeMediatorTask : NodeMediator
    {
            ...
            if (base.Linker.FromActivity.ActivityTypeDetail.ComplexType == ComplexTypeEnum.MultipleInstance)
            {
                ...
                else if (base.Linker.FromActivity.ActivityTypeDetail.MergeType == MergeTypeEnum.Parallel)
                {
                    ...
                    if (completedCount / allCount >= mainActivityInstance.CompleteOrder)
                    {
                        ...
                        mainActivityInstance.ActivityState = (short)ActivityStateEnum.Completed;
                        base.ActivityInstanceManager.Update(mainActivityInstance, session);
                        var allactivities = base.ActivityInstanceManager.GetActivityByState(mainActivityInstance.ProcessGUID, mainActivityInstance.ProcessInstanceID, mainActivityInstance.ActivityGUID, ActivityStateEnum.Ready, session);
                        //设置其它活动节点的状态为完成状态
                        if (allactivities != null && allactivities.Count > 0)
                        {
                            foreach (var acti in allactivities)
                            {
                                base.ActivityInstanceManager.Complete(acti.ID,
                                activityResource.AppRunner,
                                session);
                            }
                        }
            ...
            IDbSession session)
        {
            ...
            else
            {
                ...
                toActivityInstance.ApprovePageUrl = urlm.GetFlowStepUrl(processInstance.ProcessGUID, toActivity.ActivityGUID);
            ...
            IDbSession session)
        {
            ...
            for (short i = 0; i < plist.Count; i++)
            {
                ...
                entity.ApprovePageUrl = toActivityInstance.ApprovePageUrl;
                entity.ViewPageUrl = toActivityInstance.ViewPageUrl;
                //插入活动实例数据，并返回活动实例ID
