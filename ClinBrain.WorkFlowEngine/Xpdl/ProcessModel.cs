    ...
    public class ProcessModel
    {
        ...
        internal XmlDocument XmlProcessDefinition
        {
            get
            {
                ...
                xmlDoc.LoadXml(ProcessEntity.XmlConfig);
                return xmlDoc;
            }
        }
        internal XmlDocument XmlProcessDefinitionOld
        {
            get
            {
                //if (CachedHelper.GetXpdlCache(ProcessGUID) == null)  kevin 暂时不用cache
                {
        ...
        internal XmlDocument GetProcessXmlDocument()
        {
            ...
            if (XPDLReader.IsReadable() == false)
            {
                ...
                string physicalFileName = System.AppDomain.CurrentDomain.BaseDirectory + serverPath + "\\" + filePath;
        ...
        #region 获取流程节点对应的URL信息 Kevin
        internal string GetFlowStartUrl()
        {
            string nodeType = "StartNode";
            XmlNode startTypeNode = GetXmlActivityTypeNodeFromXmlFile(nodeType);
            return XMLHelper.GetXmlAttribute(startTypeNode.ParentNode, "url");
        }

        internal string GetFlowStepUrl(string activityGUID)
        {
            ...
            return XMLHelper.GetXmlAttribute(xmlNode, "url");
        }

        internal string GetViewStepUrl(string activityGUID)
        {
            XmlNode xmlNode = XMLHelper.GetXmlNodeByXpath(XmlProcessDefinition,
                    string.Format("{0}[@id='" + activityGUID + "']", XPDLDefinition.StrXmlActivityPath));
            return XMLHelper.GetXmlAttribute(xmlNode, "viewUrl");
        }

        #endregion

        #region 获取流程跳转结束后执行脚本 Kevin0701

        internal string[] GetAfterTransitionAction(string transitionGUID)
        {
            string[] result = new string[2];
            XmlNode xmlNode = XMLHelper.GetXmlNodeByXpath(XmlProcessDefinition,
                    string.Format("{0}[@id='" + transitionGUID + "']", XPDLDefinition.StrXmlTransitionPath));
            XmlNode eventNode = xmlNode.SelectSingleNode("AfterSqlEvent");
            if (eventNode != null)
            {
                string connString = XMLHelper.GetXmlNodeValue(eventNode, "ConnString");
            ...
            foreach (var child in nextSteps.Root)
            {
                ...
                else
                {
                    treeNodeList.Add(new NodeView
                    {
                        ...
                        Participants = child.Activity.Participants  //added by kevin
                    });
                }
            }
            return treeNodeList;
        ...
        /// <returns></returns>
        private Role GetRoleFromXmlFile(string performerGUID)   //kevin
        {
            XmlNode performerNode = GetXmlParticipantNodeFromXmlFile(performerGUID);
            if (XMLHelper.GetXmlAttribute(performerNode, "type") == "Role")
            {
        ...
        internal IList<Role> GetActivityRoles(string activityGUID)
        {
            ...
            if (performersNode != null)
            {
                foreach (XmlNode performer in performersNode.ChildNodes)
                {
                    ...
                    var role = GetRoleFromXmlFile(performerGUID);
                    if (role != null)
                    {
                        roles.Add(role);
                    }
        ...
        private bool ParseCondition(TransitionEntity transition, IDictionary<string, string> conditionKeyValuePair)
        {
            ...
            {
                expressionReplaced = ReplaceParameterToValue(expression, conditionKeyValuePair);
            }
